...
import 'package:bd_mtur/controllers/register_controller.dart';
...
class _RegisterState extends State<Register> {
  late UserController _controllerUser;
  late TopicInterestController _controllerTopicInterest;
  ...
  void initState() {
    ...
    super.initState();
    _controllerUser = UserController();
    _controllerTopicInterest = TopicInterestController();
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    padding: EdgeInsets.only(top: 10),
                    child: InputField(
                      title: 'Informe o email cadastrado na UNASUS',
                      obscure: false,
                  ...
                  Padding(
                    padding: EdgeInsets.only(top: 10),
                    child: InputField(
                      title: 'Informe a sua senha de acesso',
                      obscure: true,
                  ...
                  Padding(
                    padding: EdgeInsets.only(top: 10),
                    child: Text(
                      'Confirme a sua senha de acesso',
                      style: AppTextStyles.labelContentTitleText,
                    ),
                  ),
                  Padding(
                    padding: EdgeInsets.only(top: 10),
                    child: InputField(
                      title: 'Informe a senha de acesso novamente',
                      obscure: true,
                      ...
                      onPressed: () async {
                        if (_key.currentState!.validate()) {
                          try {
                            ...
                            await _controllerUser.register(
                              ...
                              onPasswordConfirmFail:
                                  errorRegisterPasswordConfirm,
                              onAcceptedTermsFail: errorRegisterAcceptedTerms,
                            );
                          } catch (e) {
                            if (e == "Usuário já existe") {
                              errorRegisterEmail();
                            } else if (e == SocketException) {
                              errorRegisterNoConnection();
                            }
                          }
  ...
  Future<bool> newUserSuccessfull() async {
    return PopUpMessage(context, "Cadastro realizado com sucesso",
        "Usuário cadastrado com sucesso. Agora você pode acessar o aplicativo.");
  }

  Future<bool> newUserFail() async {
    return PopUpMessage(context, "Erro ao cadastrar",
        "Erro ao cadastrar o usuário. Tente novamente mais tarde.");
  }

  Future<bool> errorRegisterEmail() async {
    return PopUpAction(
        context,
        "Erro ao cadastrar",
        "O e-mail cadastrado já está em uso. Por favor, acesse o aplicativo usando seu nome de usuário e senha UNASUS. Se você não lembra seu login e senha, utilize a função “Esqueci minha senha”.",
        "Esqueci minha senha");
  }

  Future<bool> errorRegisterNoData() async {
    return PopUpMessage(context, "Erro ao cadastrar",
            "Dados obrigatórios de cadastro não preenchidos. Por favor, informe o nome, e-mail e a senha de acesso.") ??
        false;
  }

  Future<bool> errorRegisterAcceptedTerms() async {
    return PopUpMessage(context, "Aceite os termos",
            "Para concluir o cadastro é necessário aceitar os termos e as condições de uso.") ??
        false;
  }

  Future<bool> errorRegisterPasswordConfirm() async {
  ...
  Future<bool> errorRegisterNoConnection() async {
